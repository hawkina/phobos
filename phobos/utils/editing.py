...
def parentObjectsTo(objects, parent, clear=False):
    ...
    if parent.phobostype == 'link':
        ...
        armature = bpy.data.objects['armature_object'] # find armature
        bpy.context.scene.objects.active = armature # set the object of armature to active
        bpy.ops.object.mode_set(mode='EDIT', toggle=False) # go into edit mode
        
        armature.data.edit_bones.active = armature.data.edit_bones[parent.name] # select the parent bone
